...
: iss(iss)
{
    ...
    this->iss.csr.mstatus.register_callback(std::bind(&Core::mstatus_update, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
    this->iss.csr.sstatus.register_callback(std::bind(&Core::sstatus_update, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));

    this->iss.traces.new_trace_event("thread_lifecycle", &this->event_thread_lifecycle,
        sizeof(iss_reg_t) * 8);
    this->iss.traces.new_trace_event("thread_current", &this->event_thread_current,
        sizeof(iss_reg_t) * 8);
    this->iss.traces.new_trace_event("insn_is_jal_noreg", &this->event_jal, 1);
    this->iss.traces.new_trace_event("insn_is_jal_reg", &this->event_jalr, 1);
}
...
bool Core::mstatus_update(iss_insn_t *insn, bool is_write, iss_reg_t &value)
{
    if (is_write)
    {
...
bool Core::sstatus_update(iss_insn_t *insn, bool is_write, iss_reg_t &value)
{
    this->iss.timing.stall_insn_dependency_account(4);
