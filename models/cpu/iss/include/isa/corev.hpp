...
static inline iss_reg_t corev_hwloop_check_exec(Iss *iss, iss_insn_t *insn, iss_reg_t pc)
{
    ...
    if (iss->csr.hwloop_regs[COREV_HWLOOP_LPCOUNT0] && iss->csr.hwloop_regs[COREV_HWLOOP_LPEND0] == pc)
    {
        ...
        if (iss->csr.hwloop_regs[COREV_HWLOOP_LPCOUNT0])
        {
    ...
    if (iss->csr.hwloop_regs[COREV_HWLOOP_LPCOUNT1] && iss->csr.hwloop_regs[COREV_HWLOOP_LPEND1] == pc)
    {
        ...
        if (iss->csr.hwloop_regs[COREV_HWLOOP_LPCOUNT1])
        {
            // Remember next instruction in case the current instruction is replayed
...
{
    iss->csr.hwloop_regs[COREV_HWLOOP_LPCOUNT(index)] = count;
}
...
/* ISA-9: p.bitrev label alias — isa_gen derives handler name from label,
